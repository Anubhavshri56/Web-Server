...
int ii=0;
if(this->url[0]=='/') ii=1;
if(strcmp(this->url+ii,request->getResource())==0)
{
this->ptrOnRequest(request,response);
if(request->hasData()) request->release();
printf("Sending processed page\n");
//char tmp[501];
//sprintf(tmp,"<!DOCTYPE HTML><html lang='en'><head><meta charset='utf-8'><title>TM Web Projector</title></head><body><h3 style='color:green'>Resource/%s processed </h3></body></html>",request->getResource());
//sprintf(responseBuffer,"HTTP/1.1 200 OK\ncontent-type:text/html\ncontent-length:%d\nConnection: close\n\n",strlen(tmp));
